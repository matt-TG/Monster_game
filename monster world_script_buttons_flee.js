...
//1. More values to player and enemies? 2. A way to use multiple items?
...
useItem.onmouseover=function(){
  useItem.title="type item + enter=> potion=potion || potionbig=potionb || shieldrepair=shield || powerpotion=power";
}
...
function result() {
  ...
  if(eventTargetHolder.dataset.play=="true"){
    ...
    if(shield.style.backgroundColor=="green" && player.shieldUse>0){
    ...
    if(monsters.length==1 && hydraRegenRan>0.96 && hydraRegen==1){
    ...
    if(monsters.length>=2){
      ...
      if(hydraRegenRan>0.96 && hydraRegen==1 && regenMessage){
...
function buildB() {
  ...
  shield.onmouseover=function(){
    shield.title="Use shield by clicking on this button (green=activated)";
  }
...
function Items() {
  ...
  if(useItem.value.toLowerCase()=="shield"){
    ...
    inventoryuse.style.visibility="visible";
    console.log(shield.style.backgroundImage);
